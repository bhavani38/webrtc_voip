...
io.on("connection", function (socket) {
  ...
  socket.on("offer", function (event) {
    ...
    io.to(event.receiver).emit("offer", {
      event: event.sdp,
      caller: socket.id,
    });
  });
  ...
  // Example socket event for disconnecting
  socket.on("disconnect", (data) => {
    // Handle the disconnection logic, e.g., inform the other peer
    console.log("Disconnected from peer: " + data.id);
  });
});
