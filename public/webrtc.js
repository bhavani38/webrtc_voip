...
// Add this event listener for the disconnect button
$("#disconnectBtn").click(function () {
  disconnectCall();
});

function disconnectCall() {
  // Stop the local stream (video/audio) tracks
  if (localStream) {
    localStream.getTracks().forEach((track) => track.stop());
    localVideo.srcObject = null; // Remove video from the UI
  }

  // Stop the remote stream (video/audio) tracks
  if (remoteStream) {
    remoteStream.getTracks().forEach((track) => track.stop());
    remoteVideo.srcObject = null; // Remove remote video from the UI
  }
  ...
  socket.emit("disconnect", {
    // Emit a disconnect signal to the server if necessary
    id: isCaller ? receiver : caller,
  });
